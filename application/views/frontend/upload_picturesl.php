...
<link href="<?php echo $this->config->item('theme_url') ?>css/newcss/popupwindow.css" type="text/css" rel="stylesheet" />
<script src="<?php echo $this->config->item('theme_url') ?>js/newjs/popupwindow.js"></script>
<script src="<?php echo $this->config->item('theme_url') ?>js/newjs/demo.js"></script>
...
$(document).ready(function() {
	...
	var forpage = $("#pageselector_f").val();
     ...
     function text_height_weidht_update(txttop,txtleft){
        var forpage = $("#pageselector_f").val();
	var  postData = {"forpage":forpage,"txt_top":txttop,"txt_left":txtleft}    
	$.ajax({
	...
	{
		case 0:
		$('.mid_container h2').html('Create Front');break;
		
		case 1:
		$('.mid_container h2').html('Create Side');break;
		
		case 2:
		$('.mid_container h2').html('Create Middle');;break;	
		
		case 3:
		$('.mid_container h2').html('Create Back');break;		
...
*/	
	...
	$.ajax({
		    ...
		    success: function(response){
					    ...
					    var h = parseInt(360)-parseInt($("div#img_txt2").height());
					    var top = parseInt(parseInt(h)/2);
                                                
                                            var left = 124;
					    
                                            $("div#txt2").css('left',left);
                                            $("div#txt2").css("top",top);
					    
                                            text_height_weidht_update(top,left); 

							$("a.pop-up-close-trigger").trigger("click");
   ...
   //image upload
    $('#image_file').live('change', function() { 
		...
		$("#imageform").ajaxForm({
			...
			aspectRatio: 200 / obj.newHeight_0,
                        ...
                        var h = parseInt(360)-parseInt(obj.newHeight_0);
                        var top  = parseInt(parseInt(h)/2);
			 $("#rnd").css("left",152);
                         $("#rnd").css("top",top);
	...
	$.ajax({
		    ...
		    success: function(response){
				    ...
				    if (obj.recentCardDetails.image0 == '')
				    {
				    var imgpath_0 = '<?php echo $this->config->item('theme_url')?>newimages/image_select.jpg';
				    $("#imgthumb0 img").attr("src",imgpath_0);
    ...
    var ihas = $("img#img_rnd").attr("src");
    var thas = $("div#img_txt2>span").text();
    
    if (thas != '' && thas != '')  $(".pagectrl:eq(3)").trigger("click");
    if (thas == '')  $(".pagectrl:eq(2)").trigger("click");
    if (ihas =='' && thas == '')  $(".pagectrl:eq(0)").trigger("click");
});
...
function validity()
{
	...
	if (imgvalue == '') msg +='You can\'t create a card without image\n';
	if (txtvalue == '') msg +='You can\'t create a card without text';
...
<div class="mid_container" style="padding: 60px 0px; overflow: hidden;">
	<h2>Create Front</h2>
	<div class="landscape_left">

		<div class="landscape_cardview">
			
        	
		<div class="view1 page" style="display: block;">
		<div class="main_displaybox" style="display: block;">
			
		<div id="rnd" style="cursor: move; text-align: center; vertical-align: middle; display:table; left: <?php echo (isset($recentCardDetails['img0_left']) && $recentCardDetails['img0_left'] != '') ? $recentCardDetails['img0_left'] : '0px';?>; top: <?php echo (isset($recentCardDetails['img0_top']) && $recentCardDetails['img0_top'] != '') ? $recentCardDetails['img0_top'] : '0px';?>;">
		...
		<div class="view3 page" style="display: none;">
		    ...
		    <div class="main_displaybox" style="width: 504px; z-index: 999;">
	    ...
	    <div class="view4 page" style="display: none;">
            	<div class="main_displaybox">
                	<div class="lastlogo2">
                    	<div class="logoview2"><a href="#"><img src="<?php echo $this->config->item('theme_url')?>images/logo.png" border="0" alt="" /></a></div>
                        <div><a href="#">www.cardsleaf.com</a></div>
                    </div>
                </div>
            </div>
        </div>
			...
			<div class="preview_card">
				<ul>
					<li class="active pagectrl"><a style="cursor: pointer;"><img src="<?php echo $this->config->item('theme_url')?>newimages/front.jpg" border="0" alt="" /></a></li>
					<li class="pagectrl"><a style="cursor: pointer;"><img src="<?php echo $this->config->item('theme_url')?>newimages/side.jpg" border="0" alt="" /></a></li>
					<li class="pagectrl"><a style="cursor: pointer;"><img src="<?php echo $this->config->item('theme_url')?>newimages/middle.jpg" border="0" alt="" /></a></li>
					<li class="pagectrl"><a style="cursor: pointer;"><img src="<?php echo $this->config->item('theme_url')?>newimages/back.jpg" border="0" alt="" /></a></li>
				</ul>
				<div class="clear"></div>
			</div>			
	...
	<div class="rightcardpl">
	    ...
	    <?php } else {?>
	    <img width="80px" height="80px" src="<?php echo $this->config->item('theme_url')?>newimages/image_select.jpg" border="0" alt="" />
	    <?php } ?>
	    </span>
	    </div>
	    ...
	    -->
            ...
            <div class="uploadoption" style="display: block;" id="upload_switch">
            	<!-- p>* You are allowed to upload upto  4mb image</p -->
		<ul>
			<li>
				<a id="upload_select_img" style="cursor: pointer;">Select image</a>
			</li>
			<li> | </li>
			<li><a id="del_img" style="cursor: pointer;">Delete</a></li></br>
		</ul>
		</div>
	...
	<div id="make" style="display: none;" class="make"><a href="javascript:void()"><div  class="signup_btn" >Make it Now</div></a></div>
	<div class="clear"></div>

            <!--div><input name="make"  type="button" value="Make it now" class="cardbutton"  /></div-->
		
		
<div id="text_block" style="display: none;">		
	<div class="make"><a href="#pop-up-1" id="open-pop-up-1"><div class="signup_btn" name="set_text" id="set_text">Add Text</div></a></div>		
	<div class="clear"></div>		
	<div class="seen">
	<div class="size">
		<label>Size:</label>
		<input type="text" class="size_input" name="fsize" id="fsize" value="<?php echo (isset($recentCardDetails['txt2_fsize']) && $recentCardDetails['txt2_fsize'] != '') ? intVal($recentCardDetails['txt2_fsize']) : '16.0';?>" />
		</div><!--size-->

		<div class="up_down spinner">
		<a href="javascript:void(0)" id="fsizeup"><img src="<?php echo $this->config->item('theme_url') ?>newimages/up_arrow.png"></a>
		<a href="javascript:void(0)" id="fsizedn"><img src="<?php echo $this->config->item('theme_url') ?>newimages/down_arrow.png"></a>
		</div>
	</div>		
	<div class="clear"></div>		
</div>		
	    ...
	    <!--div id="next_prev"><input name="prev" id="prev" type="button" value="Previous" class="cardbutton" />
		<input name="next" id="next" type="button" value="Next" class="cardbutton" />
	    </div-->

		<div class="slide clearfix">
			<div class="prev"><a id="prev" href="javascript:void(0)"><img src="<?php echo $this->config->item('theme_url') ?>newimages/prev.png"></a></div>
			<div class="next"><a id="next" href="javascript:void(0)"><img src="<?php echo $this->config->item('theme_url') ?>newimages/next.png"></a></div>
		</div>		
...
<div id="pop-up-1" class="pop-up-display-content pop_head">
<form id="textform" method="post" enctype="multipart/form-data" action='' style="margin: 0; padding: 0;">
 <input type="hidden" name="pageselector_f" id="pageselector_f" value="0">
	<div class="pop">
	<h3>Add Your Text</h3>
	<div class="txt">
	<textarea name="cardtext" id="cardtext" cols="" rows="" class="area" placeholder="200 characters maximum"><?php echo (isset($recentCardDetails['txt2']) && $recentCardDetails['txt2'] != '') ? $recentCardDetails['txt2'] : ''; ?></textarea></div><!--txt-->
	<div class="note">Note: 200 character(s) left</div>
	<div class="signup" name="text_btn" id="text_btn"><a href="javascript:void(0)"><div class="signup_btn">Add</div></a>
	</div>
	<div class="clear"></div>
	</div><!--pop-->
</form>	
