...
class member extends CI_Controller 
{
	...
	function delete()
	{
		...
		if($id)
		{
		    $data['user_info'] = $this->membermodel->getMemberById($id);
			unlink('./uploads/member_photo/large/'.$data['user_info'][0]['user_profile_photo']);
			unlink('./uploads/member_photo/thumb/'.$data['user_info'][0]['user_profile_photo']);

			$this->membermodel->deletememberevents($id);
			$this->membermodel->deletememberfriends($id);
			$this->membermodel->deletememberfriends2($id);
			$this->membermodel->deletememberjournals($id);
			$this->membermodel->deletemembervideos($id);
			$this->membermodel->deletemembermeetups($id);
			$this->membermodel->deletememberalbums($id);
			$this->membermodel->deletememberphotos($id);
			$this->membermodel->deletememberplantrip($id);
			$this->membermodel->deletememberstatus($id);
			$this->membermodel->deletemembersuggestions($id);
			$this->membermodel->deletememberwallphotos($id);
			$this->membermodel->deletemembereventjoinpeople($id);
			$this->membermodel->deletememberdiscuss($id);
			$this->membermodel->deletememberprivacysettings($id);
			$this->membermodel->deletememberprivacysettings2($id);
			$this->membermodel->deletememberactivity($id);
